...
- **Collections** are curated groupings — a *manual* collection holds up to **50 items of one content type**; dynamic collections are query-driven — used to bind a component (list, carousel) to content without hard-coding item IDs.
...
Enhanced workspaces are the default and the only place taxonomy, variants and cross-workspace sharing work — classic workspaces are frozen feature-wise. Enhanced LWR sites are deployed as the **`DigitalExperienceBundle`** metadata type, not `ExperienceBundle` — the deployment trap in [18](18-experience-cloud-devops.md). Delivery detail: [AI_Data/05-release-radar/README.md](../../AI_Data/05-release-radar/README.md).
...
- **Manual collections cap at 50 items, single type.** Larger or mixed-type lists need dynamic collections.
...
Q: What is the limit on a manual collection?
A: 50 items of a single content type. Larger or mixed-type groupings need a dynamic/query-driven collection.
