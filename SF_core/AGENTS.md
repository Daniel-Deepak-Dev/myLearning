...
- **Out of scope:** Aura, Visualforce, OmniStudio, Analytics. Agentforce lives in [../SF_Agentforce/](../SF_Agentforce/INDEX.md), Data 360 in [../SF_Data_360/](../SF_Data_360/INDEX.md), **Experience Cloud in [../SF_Experience_Cloud/](../SF_Experience_Cloud/INDEX.md)** — it was area 05 here until 2026-09-19 — **Service Cloud in [../SF_Service/](../SF_Service/INDEX.md)** and **Sales Cloud in [../SF_Sales/](../SF_Sales/INDEX.md)**, both since 2026-09-24.
- **The line is drawn by artefact.** `SF_core/` owns class names, signatures, exceptions, limits and entitlement. The AI vaults own prompt authoring, agent reasoning and Trust Layer policy; `SF_Experience_Cloud/` owns what a site container does to them. **Ask: strip the product out — is the sentence still true?** *No Agentforce in it? No Experience Cloud site in it? No Service Cloud in it? No Sales Cloud in it?* Still true → it belongs here. Falls apart → it belongs there.
...
- **The Service Cloud seam, concretely.** Queues, assignment rules and escalation rules are here, in [01-admin-and-declarative-platform/](01-admin-and-declarative-platform/INDEX.md) — they also work on Lead, Task and custom objects. *How Omni-Channel pushes a queue's work to an available rep* is there. Custom Lightning Types are here; *which Enhanced Chat version renders them* is there.
- **The Sales Cloud seam, concretely.** The share rows that account teams, opportunity teams and territories add are here, in [07-security · 10](07-security-and-sharing/10-teams-territories-and-account-sharing.md); *team roles, splits, territory types and forecasts* are there. The Lead → Opportunity → PricebookEntry object graph is here, in [08-data · 04](08-data-modeling-and-large-data-volumes/04-standard-crm-object-map.md); *what lead conversion maps and how a price book governs line items* is there. Lead assignment rules stay in `01-admin · 11` with the other assignment rules.
- **Link both ways.** When a note here gains a link to `SF_Agentforce/`, `SF_Data_360/`, `SF_Experience_Cloud/`, `SF_Service/` or `SF_Sales/`, add the return link in that file in the same edit. Same-vault links do not need one — Obsidian's Backlinks panel shows those for free.
