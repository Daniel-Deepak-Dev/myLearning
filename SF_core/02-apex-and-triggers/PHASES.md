...
**The `AI_Data/` boundary was the binding constraint, not an afterthought.** [../README.md](../README.md) line 32 sends Agentforce content to the sibling vault, which is why these four notes are scoped to class names, signatures, exceptions, limits and entitlement, and cross-link rather than restate: prompt authoring to [AI_Data · Prompt Builder](../../AI_Data/02-salesforce-ai/03-prompt-builder/notes.md), injection-and-masking policy to [AI_Data · Einstein Trust Layer](../../AI_Data/02-salesforce-ai/04-einstein-trust-layer/notes.md), model choice to [AI_Data · Model Builder & BYOM](../../AI_Data/02-salesforce-ai/06-model-builder-byom/notes.md). The rule that kept it honest while writing: if a sentence would still be true with no Apex in it, it belongs in the other vault.
