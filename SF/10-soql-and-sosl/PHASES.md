...
**SOSL was the clearest gap in the vault.** The entire coverage was a four-row comparison table and about four bullets inside `02-apex · 04`. Grepping all of `SF/` for `SEARCH GROUP`, `WITH DATA CATEGORY`, `WITH SNIPPET`, `SPELL_CORRECTION`, `WITH HIGHLIGHT` and `toLabel` returned **zero hits each**. Two notes now carry it: [08](08-sosl-mechanics-and-the-search-index.md) for the engine and statement, [09](09-sosl-search-modifiers-and-relevance.md) for the modifiers and matching behaviour.
