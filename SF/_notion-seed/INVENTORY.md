...
**→ 08 Data modeling & LDV** — *all three inspected in phase 14*

- `QUERY PLAN` (**2025**) → `09-query-plan-and-performance-tuning` — ✅ **genuinely current**, one of the few in the corpus. Worked example is correct (`TableScan` 1.5 → `Index` 0.5) and the standard-index field list is accurate. ⚠️ **One error, corrected inline:** it says custom indexes are made *"by requesting Salesforce Support or enabling them in Setup"* — **there is no Setup screen for custom indexes**. It also never states that **1.0 is the threshold**.
- `Optimization work` (2023) → `08-indexes-and-query-selectivity`, **not 09/10 as the phase plan guessed**. Content is one artefact: an **ultimate-parent formula field**, eleven nested `IF`s walking `Parent.Parent…` up an Account hierarchy. ✅ **Best worked example in the area** — cross-object, therefore non-deterministic and **permanently non-indexable**, while sitting at both the 10-relationship traversal ceiling and the 10-unique-relationships-per-object ceiling. Harvested as the argument for materialising instead of computing at read time.
- `Lead Broker Field Migration` (2023) → `02-relationships-deep-dive`. ⚠️ **Misfiled: not a data-migration note.** It is a deployment checklist — report types missing in Stage, fields absent in devsd and QA. One durable lesson survives: its first task is *"change the field label and relationship label and rel api name"*, and **report types are built on relationship paths**, so renaming a relationship commits you to a report-type deployment. **Its natural long-term home is area 09 (DevOps)**, not here.
