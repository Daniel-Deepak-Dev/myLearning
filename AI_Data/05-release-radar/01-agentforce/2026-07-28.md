...
**Second pass — structural gaps**

- [Second pass: four topics the study base never had](#second-pass-four-topics-the-study-base-never-had)
...
## Second pass: four topics the study base never had

_Added later on 2026-07-28, after a question about whether any Agentforce topics were missing. **The news finding stands unchanged: nothing was announced July 25–28**, and an independent re-check confirmed it — the Salesforce Developers blog still ends at June 24, Winter '27 notes are still unpublished. What the second pass found instead was **structural**: four substantial parts of the 2026 Agentforce surface that the ten study topics had never covered, verified by grepping the whole study base. None of them is breaking news, which is exactly why a daily scan window kept missing them._

| Gap | Coverage before this pass | Now written up at |
|---|---|---|
| **MuleSoft Agent Fabric** | **Zero mentions repo-wide** — this radar included — despite launching September 2025 | [02-salesforce-ai/11-agent-fabric-and-interop](../../02-salesforce-ai/11-agent-fabric-and-interop/notes.md) · [developer-tooling-and-apis.md](../developer-tooling-and-apis.md) |
| **Agentforce Voice / Contact Center** | One passing bullet; "Contact Center" absent outside a field note, despite **GA since 2026-02-23** | [02-salesforce-ai/12-voice-and-contact-center](../../02-salesforce-ai/12-voice-and-contact-center/notes.md) · [agentforce-platform.md](../agentforce-platform.md) |
| **ADLC + Agentforce DX** | Scattered passing references in three files; the first-party command sequence never captured | [02-salesforce-ai/13-adlc-and-agentforce-dx](../../02-salesforce-ai/13-adlc-and-agentforce-dx/notes.md) · [developer-tooling-and-apis.md](../developer-tooling-and-apis.md) |
| **Prebuilt agents / buy vs build** | Commerce and Contact Center absent outside this radar; no decision framework anywhere | [02-salesforce-ai/14-prebuilt-agents-and-buy-vs-build](../../02-salesforce-ai/14-prebuilt-agents-and-buy-vs-build/notes.md) |

**Four findings worth carrying out of this pass, independent of the topic write-ups:**

1. **Flex Gateway was renamed Omni Gateway** — same MuleSoft runtime, expanded to govern AI/MCP/A2A traffic, non-breaking and cosmetic (1.13.0). Both names circulate; Agent Fabric launch coverage still says Flex.
2. **"Agent Script" names two products** — the Agentforce authoring language (Apache 2.0) and "Agent Script for Agent Broker", the April 2026 guided-determinism feature. Ask which one is meant.
3. **Voice has two billing models, not one** — 30 credits per action *or* ~60 credits per minute. A five-minute call with three actions prices at ~90 credits one way and ~300 the other. This can move a cost estimate by 3×.
4. **The Agentforce Specialist domain weights are in doubt.** Study resources report **Multi-Agent Interoperability at 15%** and a **Development Lifecycle and Observability domain at 15–20%** that this radar never recorded — against the ~5% and no-lifecycle-domain figures recorded on [July 26](../pricing-and-certification.md). Sources disagree with each other; the official guide could not be read first-party. **If the higher numbers are right, ~30% of the exam sits in two areas the study plan treated as peripheral.** Both readings are now recorded side by side in [the exam guide](../../02-salesforce-ai/_cert-agentforce-specialist/exam-guide.md), flagged unverified.

**Status:** Second pass completed 2026-07-28. **No new announcements found — the news windows remain empty.** Four structural gaps closed; four topic folders added (11–14); two radar corrections flagged (voice billing, exam weights). Two new open questions logged in the [README](../README.md): Agent Broker's real status, and the official exam weights.

**Sources:** [MuleSoft Agent Fabric Overview](https://docs.mulesoft.com/general/agent-fabric-overview) · [Salesforce Advances Agent Fabric (2026-04-15)](https://www.salesforce.com/news/stories/agent-fabric-control-plane-announcement/) · [Master the Agentic Development Lifecycle for Agentforce](https://developer.salesforce.com/blogs/2026/06/master-the-agentic-development-lifecycle-for-agentforce) · [Enterprise Connect 2026: Agentforce Contact Center (UC Today)](https://www.uctoday.com/unified-communications/salesforce-agentforce-contact-center/) · [Agentforce Voice overview (developer guide)](https://developer.salesforce.com/docs/ai/agentforce-partner/guide/agentforce-voice-overview.html) · [The Agentforce Specialist Exam Has Evolved (FocusOnForce)](https://focusonforce.com/blog/agentforce-specialist-exam-update/)
