...
34 topics across 6 runs — **phases 03–05, 20, 21 and 23 complete, the area is done.** Master plan: [../PHASES.md](../PHASES.md) · standing rules there apply to every phase.

> **The area-wide constraint.** Three defaults flipped at API 67.0 — user mode, `with sharing`, `WITH SECURITY_ENFORCED` retired. Phase 04 owns them, but **every phase must be written as if they are already true.** Never show a code sample that relies on the old defaults without labelling it. Anchor: [RELEASE-RADAR/trust-security-and-governance.md](../../RELEASE-RADAR/trust-security-and-governance.md).
...
## Phase 23 — Advanced AI in Apex · 4 files ✅

**The third maintenance run, and the same coverage trigger as phase 21 pointed at a different surface.** The area's AI coverage stopped at the declarative seam: [22](22-invocable-apex-and-agentforce-actions.md) owns `@InvocableMethod` — the annotation that lets *something else* call Apex — and then hands off to `AI_Data/`. Nothing in either vault covered the Apex that calls a *model*. A grep of the whole repo for `ConnectApi`, `aiplatform`, `ModelsAPI`, `EinsteinLLM`, `createChatGenerations`, `createEmbeddings`, `generateMessages` and `capabilityType` returned **zero hits on every one of them**.

```
31-apex-grounded-prompt-templates.md            🆕
32-invoking-prompt-templates-from-apex.md       🆕
33-models-api-in-apex.md                        🆕
34-testing-ai-apex-and-mocking-llms.md          🆕
```

**Appended, not renumbered** — the phase-21 position, unchanged, and for the same reason: 31 belongs pedagogically beside [22](22-invocable-apex-and-agentforce-actions.md) and moving it there would rewrite inbound links from four other areas.

**31 — the finding is that `capabilityType` makes 22's annotation a different feature.** Every write-up treats Apex grounding as a Prompt Builder topic and shows the class without saying what changed about the annotation. Add `capabilityType` and the platform stops calling your method *as an action* and starts calling it *during prompt resolution*: the return value is no longer a result, it is a `String` spliced into the prompt, and the contract is a `Response` class exposing `@InvocableVariable public String Prompt` — that exact field name. Four capability types: `PromptTemplateType://einstein_gpt__salesEmail`, `__fieldCompletion`, `__recordSummary`, and `FlexTemplate://<template_API_Name>`, where the Flex form binds to a template's **API name** rather than a type.

**The 66.0 constructor rule reaches further than anyone documents it.** A grounding `Request` class is an invocable parameter class, so the API 66.0 no-arg-constructor Release Update applies to it exactly as it applies to an action's input — but every published example frames that rule as an *agent action* concern. Same failure shape as [22](22-invocable-apex-and-agentforce-actions.md) and [28](28-dependency-injection-and-pluggable-apex.md): a parameterised constructor removes the compiler-generated default, and prompt resolution starts failing at runtime with no compile error.

**32 — two corrections, one of them about an absence.** The `ConnectApi.EinsteinLLM` class reference at 67.0 lists **exactly two static methods**: `generateMessagesForPromptTemplate` (API 60.0) and `getPromptTemplates` (API 62.0). The raw `generateMessages`/`EinsteinLLMGenerationsInput` path that most 2024 tutorials lead with **is not on it** — checked twice against the reference, which carries no deprecation notice, so the note states the absence and hedges the retirement rather than asserting one. The second correction was to this phase's own plan: `numGenerations` and `temperature` are **not** properties of `EinsteinPromptTemplateGenerationsInput`. The real surface is `inputParams`, `additionalConfig` and `isPreview` (**all three required**), plus optional `citationMode`, `outputLanguage` and `tags`. `applicationName` is a magic string, `PromptTemplateGenerationsInvocable`, and `isPreview = true` resolves the template without spending a model request — which turns out to be the only assertion 34 can make without a model.

**33 — the shape of the class is the tell.** `aiplatform.ModelsAPI` is generated from an **External Services** definition, which explains the `_Request`/`_Response`/`_ResponseException` triad per method, the `Code200` property standing in for an HTTP status, and the `body` assigned separately from the request wrapper. Two consequences worth more than the API tour: the methods are **instance methods on `new aiplatform.ModelsAPI()`**, not static like `ConnectApi` — so the obvious static call does not compile — and because it is External Services underneath, every call is **an Apex callout** drawing on the same per-transaction budget as `HttpRequest` → [19](19-callouts-named-credentials-and-http-in-apex.md). Entitlement was checked per the standing rule rather than assumed: the docs say requests "are subject to Salesforce's usage and billing rates for **Einstein Requests**", so the note states that and sends rate-card detail to [07-security · 02](../07-security-and-sharing/02-licences-and-what-they-gate.md) rather than restating a SKU that will move.

**34 is why this is a phase and not three reference pages.** Three standard mocking routes fail on `ConnectApi.EinsteinLLM`, each for a different reason: `HttpCalloutMock` does not apply because a ConnectApi call is not an `HttpRequest`; the documented ConnectApi convention of a `setTest`-prefixed registration method does not apply because **`EinsteinLLM` publishes none**; and the Stub API cannot reach it because the method is **static and out-of-namespace** — two entries on [21](21-apex-testing-advanced-and-mocking.md)'s restriction list at once. What remains is [28](28-dependency-injection-and-pluggable-apex.md)'s wrap-and-inject, which 28 argues for as good design and which here is the only option. The same argument phase 21 made about the Stub API, with the escape hatch removed.

**34 ships with no `## 2026 currency` section, and the reason is different from 29's and 30's.** Not "nothing changed" but "the change belongs elsewhere": the one 2026-dated fact in reach, `@IntegrationTest` at Developer Preview, is already owned by [21](21-apex-testing-advanced-and-mocking.md), and the absence of `setTest*` methods is a stable property rather than a release event. Restating it would have duplicated a note two files away. That makes four notes in the vault taking the delete-the-heading option, after [25](25-platform-cache.md), [29](29-json-serialization-and-untyped-data.md) and [30](30-custom-iterators-and-iterables.md).

**A checked negative on the seed corpus.** [_notion-seed/INVENTORY.md](../_notion-seed/INVENTORY.md) line 143 routes the capture's AI pages — `Agentforce`, `Einstein GPT`, `Data Cloud`, `SF AI Org details` — explicitly to `AI_Data/`, and no seed page maps to any of these four topics. So no `> **From my notes.**` callout was available for this run, and the inventory needed no edit. Recorded so the next audit does not re-check it.

**The `AI_Data/` boundary was the binding constraint, not an afterthought.** [../README.md](../README.md) line 32 sends Agentforce content to the sibling vault, which is why these four notes are scoped to class names, signatures, exceptions, limits and entitlement, and cross-link rather than restate: prompt authoring to [SF_Agentforce · Prompt Builder](../../SF_Agentforce/INDEX.md), injection-and-masking policy to [SF_Agentforce · Einstein Trust Layer](../../SF_Agentforce/INDEX.md), model choice to [SF_Agentforce · Model Builder & BYOM](../../SF_Agentforce/INDEX.md). The rule that kept it honest while writing: if a sentence would still be true with no Apex in it, it belongs in the other vault.
