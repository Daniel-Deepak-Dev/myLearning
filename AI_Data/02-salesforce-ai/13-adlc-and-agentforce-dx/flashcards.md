...
Q: You install the managed `forcedotcom/sf-skills` library into `sf-pi` v0.272.0 and ask it a Data 360 question. Why does no `data360-*` skill fire?
A: Because `manual-only` is the **default invocation mode** for the managed library (ADR 0108). All 164 skills are installed and wired, but their descriptions are stamped `disable-model-invocation` on the effective tree, so they never enter `<available_skills>`. Enable the `data360` pack with `/sf-skills toggle`, or call it explicitly with `/skill:<name>` — which still works.

Q: In `sf-pi` v0.272.0, which of these is Pi actually wired to — the managed clone or the effective tree — and why does it matter?
A: The **effective tree** at `~/.pi/agent/sf-skills/effective/skills`. `installDefaults` removes the old clone-path entry from `settings.skills[]`. The clone stays pristine so `/sf-skills defaults update` can `git pull --ff-only`, then restamps the effective copy from `sfPi.skillInvocation`. Consequence: the clone is never the preference store, and any `SKILL.md` you edit there is discarded on the next update.

Q: A community-authored skill and a Salesforce `agentforce-*` skill are both installed under `sf-pi`'s default policy. Which one can the model invoke on its own?
A: The **community** one. `resolveInvocationMode` short-circuits `origin: "community"` to `agent-invocable` before the pack override and the `manual-only` default are consulted — so third-party skills stay auto-invocable while Salesforce's own default to manual.

Q: You cite "`sf-skills` 1.41.0 ships the `salesforce-development` plugin at 1.12.0." What is wrong with that?
A: Two things. The npm tarball contains **`skills/` only — no `plugins/` directory**, so no npm version carries plugin content at all. And at `32bf784`, the commit npm 1.41.0 declares as its `gitHead`, `plugin.json` reads **1.11.0** and `discovery.json` still reads `releaseRef: "1.38.0"` / 138. The 1.12.0 bump and the catalogue refresh are in the untagged commit `2476476` on `main`. For plugin content, cite a commit, never a tag.

Q: Does the first-party `salesforce-development` Claude Code plugin collect telemetry, and where does it buffer it?
A: Yes — **on by default**. Machine-wide state sits in `~/.sf/telemetry/` (`telemetry-config.json`, `telemetry-machine-id`, `telemetry-consent.lock`), but the event buffers are **project-local** in `.sf/` — `telemetry-buffer-*.jsonl`, `telemetry-org.json`, `telemetry-transmit.log`. Check `.gitignore` before the first run. `SF_DISABLE_TELEMETRY` and `DO_NOT_TRACK` are honoured.

Q: Your team asserts it disabled the plugin's telemetry in early August 2026. Why can't that assertion be trusted?
A: Before plugin **1.12.0** (commit `2476476`, 2026-08-21), `sf-context telemetry off` could **report success without taking effect** — it did not fail when it could not read or change state — and turning it off left already-buffered events and the transmit log on disk. The claim is unverifiable rather than false; re-run `telemetry status` on each machine. A control that cannot report its own failure is not a control.
