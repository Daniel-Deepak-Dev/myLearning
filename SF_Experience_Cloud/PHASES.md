# Phases for SF_Experience_Cloud — Experience Cloud (LWR-first)

20 topics across 2 runs, both run while this was `SF_core/05-experience-cloud-lwr/`. Master plan: [../SF_core/PHASES.md](../SF_core/PHASES.md) · standing rules there apply to every phase.

> **Runs last (phases 18–19) on purpose.** Guest hardening depends on [07-security](../SF_core/07-security-and-sharing/INDEX.md) (phases 10–11), custom components depend on [03-lwc](../SF_core/03-lwc-and-slds/INDEX.md) (phases 05–07), and site deployment depends on [09-devops](../SF_core/09-devops-sfdx-and-release-management/INDEX.md) (phases 16–17). Do not run this area early. *(Historical: the dependency held, and both phases are closed.)*
...
- **01** — the plan's ⚠️ was *"LWR is the default; Aura templates are legacy"* and asked what still forces Aura. **The answer is: most purpose-built templates.** Only Build Your Own (LWR) and Microsite (LWR) are LWR. **This is the fifth phase where the plan's own correction was the stale thing** — see [../SF_core/CURRENCY.md](../SF_core/CURRENCY.md).
...
**🆕 researched before writing:** **01**, **02** (Experience Delivery ~~**is still Beta**~~ — *true when written; **phase 19 found it discontinued in Winter '27**, and the 2026-09-20 maintenance run found phase 19 had over-read that into "LWR has no SSR" — see below* — Summer '24 onward; islands, ~~`lwr:hydrate`~~ *(the Experience Cloud gate is the `lightning__ServerRenderable` capability, not the LWR-on-Node directive)*, Cloudflare vs Akamai), **05** (`--dxp` hooks, SLDS 2 token removal), **06**, **11**, **12**.

**Seed harvest** ([../SF_core/_notion-seed/INVENTORY.md](../SF_core/_notion-seed/INVENTORY.md)) — as predicted, nearly nothing. `Exp Cloud Certification Prep` (2022) is the only relevant page and the one Experience Cloud database row is a **Visualforce** trick, out of scope. The one genuinely useful line came second-hand, via phase 10's harvest: *sharing rules and manual sharing do not support high-volume community users* — quoted in **09**, where it belongs.
...
- **18** — as planned, and it held. The deployment trap is **`ExperienceBundle` vs `DigitalExperienceBundle` + `DigitalExperienceConfig`**; phase 18 · **02** owns the distinction, this note owns what it does to a pipeline. The sharper framing carried through: **`ExperienceBundle` does not identify the runtime**, because Aura and non-enhanced LWR both use it. Depends on [09-devops · 05](../SF_core/09-devops-sfdx-and-release-management/05-metadata-api-and-deployment-mechanics.md).
- **16 — upgraded to ⚠️, and it is the phase's headline.** The plan said *"Carry forward that Experience Delivery is Beta."* **It is discontinued as of Winter '27.** The note's entire SSR framing was rebuilt around **static build + CDN with no SSR at all**. This is the **sixth** time a plan's own ⚠️ was stale — and the first time the stale correction was **one phase old**, not four.
  - **Corrected 2026-09-20, and this is now the build's clearest overcorrection.** Experience Delivery was the **Cloudflare-backed hosting tier**, not SSR itself. **Islands SSR remains a standard LWR Experience Cloud capability**, on by default on Build Your Own (LWR) standard pages, gated per page by `lightning__ServerRenderable` on the theme layout, with `lightning__ServerRenderableWithHydration` for components that stay interactive. Phase 19's "no SSR at all" reached five notes, `INDEX.md`, `GLOSSARY.md`, [../SF_core/CURRENCY.md](../SF_core/CURRENCY.md) and the vault's own `AGENTS.md` trap list before it was caught. **A phase that corrects a phase can be the one that needs correcting** — and a correction written as a sweeping negative (*"there is no X"*) propagates faster and is harder to dislodge than the error it replaced.
- **19 — upgraded to ⚠️, two directions on one page**, the same shape as [09-devops · 24](../SF_core/09-devops-sfdx-and-release-management/24-vscode-code-builder-and-tooling.md). **Messaging for In-App and Web was renamed Enhanced Chat in June 2025** (v2 in Winter '26) — a product-name error the note shipped with. And **legacy Chat / Live Agent genuinely was retired, 14 February 2026** — which the note asserted correctly but undated, the one claim shape this vault demands evidence for.
...
**Seed harvest — none, and that was correct.** [../SF_core/\_notion-seed/INVENTORY.md](../SF_core/_notion-seed/INVENTORY.md) line 121 maps only `Exp Cloud Certification Prep` (2022) and a ⛔-skip Visualforce row to this area, both already spent by phase 18. Recording zero here so a later reader does not mistake it for an omission.
