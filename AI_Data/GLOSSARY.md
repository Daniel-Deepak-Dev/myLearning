...
| Term | Definition |
|---|---|
| A2A (Agent-to-Agent Protocol) | Open protocol for agents built on different platforms to discover and call each other. Examinable: part of the Agentforce Specialist *Multi-Agent Interoperability* section (~5%). |
...
| Pay-per-resolution | Outcome-based pricing introduced with the Agentforce Help Agent: **$2 per issue resolved autonomously end-to-end**. Human escalations and negative-feedback sessions are free, and consumption during the interaction is unmetered. |
