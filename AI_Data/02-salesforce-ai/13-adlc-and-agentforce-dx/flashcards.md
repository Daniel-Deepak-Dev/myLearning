...
Q: After updating `sf-pi` to v0.272.0, an agent that used to pick the right Salesforce skill on its own stops doing it. What changed and how do you get it back?
A: The managed `forcedotcom/sf-skills` library defaults to **`manual-only`** — its skills are stamped `disable-model-invocation: true` on the effective tree, so they are reachable by `/skill:<name>` but invisible to the model. ADR 0108's reason: every description in `<available_skills>` "dominated first-turn context". Set `sfPi.skillInvocation.default` (or a pack entry) to `agent-invocable` and rerun `/sf-skills toggle`.

Q: `sf-pi` needs to stamp a flag on skills in a git clone it also has to `git pull --ff-only`. How does it avoid dirtying the checkout?
A: Two trees. The clone at `~/.pi/agent/sf-skills/forcedotcom/` is never written to; Pi is wired to a sibling **effective tree** at `~/.pi/agent/sf-skills/effective/skills`, which is deleted and re-copied from the clone on every sync and then stamped from `sfPi.skillInvocation`. The preference store is the settings file, never the files — which is why hand edits under `effective/` do not survive.

Q: You cloned `forcedotcom/sf-skills` yourself into `~/dev/sf-skills` and wired it. Why do its skills stay agent-invocable when the managed library does not?
A: `classifySkillOrigin` decides origin by **path substring** — only a path containing `/sf-skills/forcedotcom/` or `/sf-skills/effective/` counts as `salesforce`. Everything else is `community`, and `resolveInvocationMode` returns `agent-invocable` for community origin before it ever reaches the pack or global default.

Q: A CI job sets `SF_SOURCE_TRACKING_ASSUME_SYNCED=true` to save 40 minutes on deploys. What is the failure mode?
A: The variable skips the local filesystem scan against a source-tracking-enabled org — it **asserts** the org and DX project are already in sync and nothing verifies it. Salesforce's own warning is that if they are not, they "can get further out of sync". Being an env var it is inherited by every child process, so a shell profile or a CI base image turns it on for deploys nobody intended. It belongs only where the job creates the org it deploys to.

Q: You retrieve an agent with `sf project retrieve start --metadata Bot:MyAgent`, without the dependency flag. Do you get the planner bundle and plugins?
A: Yes, at API 64.0+. `retrieve/start.ts` auto-injects `rootTypesWithDependencies: ['Bot']` whenever `--metadata` names a **pseudotype** and the API version is > 63.0, then unions that with any explicit flag. `--root-type-with-dependencies` earns its keep for **`AiAgentDefinitionVersion`**, which is never auto-injected.

Q: The Winter '27 release notes name an Apex Symbol API. What is it for, and how is it different from `SymbolTable`?
A: A **Tooling API REST resource** returning detailed metadata for built-in, custom and dynamic Apex types, whose stated consumers are IDE completion, **AI agents generating Apex**, and setup agents answering questions about code. `SymbolTable` is a *field on* `ApexClass` / `ApexTrigger`, not a resource, and was not authoritative at this depth. It is API 68.0, so no org has it before the Winter '27 upgrade.
