...
**Why it matters for the study plan.** MCP and A2A moved from "adjacent Claude topic" to "examinable Salesforce topic" — so the Hosted MCP Servers work in [developer-tooling-and-apis.md](developer-tooling-and-apis.md) serves the [SF_Agentforce](../SF_Agentforce/INDEX.md) track. Keep the weighting honest though: at ~5% it's a nice overlap, not a reason to reallocate serious study time.
