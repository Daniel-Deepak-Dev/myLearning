...
Expert Apex, including the **API 67.0 security rewrite that invalidates most published examples**. **34 topics** · phases [03](PHASES.md), [04](PHASES.md), [05](PHASES.md), [20](PHASES.md), [21](PHASES.md), [23](PHASES.md) — **area complete ✅**.

> Currency: **Summer '26 (API 67.0)** · [flag legend](../README.md#flag-legend) · [what changed](../CURRENCY.md)

> ⚠️ **Three defaults flipped at 67.0** and they touch nearly every note in this area: SOQL/DML default to **user mode**; a keyword-less class defaults to **`with sharing`**; **`WITH SECURITY_ENFORCED` no longer compiles**. The anchor for all three is [RELEASE-RADAR/trust-security-and-governance.md](../../RELEASE-RADAR/trust-security-and-governance.md) — notes here must not contradict it.
...
| 31 | [Apex-grounded prompt templates](31-apex-grounded-prompt-templates.md) 🆕 | `@InvocableMethod(capabilityType=…)` — **the return value is prompt text, not a result** | 23 |
| 32 | [Invoking prompt templates from Apex](32-invoking-prompt-templates-from-apex.md) 🆕 | `ConnectApi.EinsteinLLM`; **three of six input properties are required**; `isPreview` | 23 |
| 33 | [Models API in Apex](33-models-api-in-apex.md) 🆕 | `aiplatform.ModelsAPI` — **instance methods, not static**; billed per Einstein Request | 23 |
| 34 | [Testing AI Apex & mocking LLMs](34-testing-ai-apex-and-mocking-llms.md) 🆕 | **no `setTest*`, no `HttpCalloutMock`, no Stub API** — wrap and inject or don't test | 23 |
...
- **22–23** are the bridge into [SF_Agentforce · custom-agent-actions](../../SF_Agentforce/INDEX.md) — that note owns the *agent* side (description writing, action selection, MCP exposure); **22** owns the Apex contract. The no-arg constructor rule is **API 66.0**, auto-enforced by a Summer '26 Release Update — phase 05 corrected that version across the AI notes.
...
- **31–34 are the programmatic AI surface, and 22 was only ever half of it.** 22 owns the annotation that lets *something else* call Apex; 31–34 own the Apex that calls a model. **31 is the hinge** — the same `@InvocableMethod` annotation, plus a `capabilityType`, called during prompt *resolution* rather than as an action, where the return value is prompt text instead of a result. Then **32** is Apex calling a template, **33** is Apex calling a model with no template, and **34** is why neither is testable by reflex. The division with the AI vaults holds: these four own class names, signatures, exceptions and limits, and defer prompt authoring to [SF_Agentforce · Prompt Builder & Templates](../../SF_Agentforce/prompt-builder-and-prompt-templates.md) and the type list to [SF_Agentforce · Prompt Template Types](../../SF_Agentforce/prompt-template-types.md).
- **34 is the phase's argument, not its appendix.** `ConnectApi.EinsteinLLM` defeats `HttpCalloutMock` (not an HTTP callout), the documented ConnectApi `setTest*` convention (it publishes none) and the Stub API (static, out-of-namespace) — three different mocking routes failing for three different reasons. What is left is [28](28-dependency-injection-and-pluggable-apex.md)'s wrap-and-inject, which 28 argues for as good design and 34 shows to be the only option. Read 34 straight after [21](21-apex-testing-advanced-and-mocking.md).
- **29, 30 and 34 deliberately have no `## 2026 currency` section.** For **29** this was checked rather than assumed: the `System.JSON` class reference at 67.0 lists nine methods with no version badges, and the Summer '26 Apex changes are the security flip, multiline strings, `String.template()`, `@IntegrationTest` and triggers-always-system-mode — **nothing about JSON or Apex serialization changed in the 2024–2026 window.** **30** is the same story for `Iterator`/`Iterable`, which have been stable far longer. **34** omits it for a different reason — its one 2026-dated fact, `@IntegrationTest` at Developer Preview, is already owned by [21](21-apex-testing-advanced-and-mocking.md), and the absence of `setTest*` methods on `EinsteinLLM` is a stable property rather than a change. With [25](25-platform-cache.md) that makes four notes taking the template's delete-the-heading option.
